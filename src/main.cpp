...
#if defined(NDEBUG)
# error "Cookiecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa3d18a50507937c4ffea532dd1081c3d6aa469046a0126caa6abd2ef88535ffc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cookiecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cookiecoin Signed Message:\n";
...
{
    // Cookiecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Cookiecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Cookiecoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Cookiecoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cookiecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cookiecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa3d18a50507937c4ffea532dd1081c3d6aa469046a0126caa6abd2ef88535ffc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CookieCoin - Jambyte & Tyler";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x85832a59fae4532c982e704c34bde3dc716ef818083fb946a7a21c024d73ad78"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cookiecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CookiecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CookiecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CookiecoinMiner(CWallet *pwallet)
{
    printf("CookiecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cookiecoin-miner");
    ...
    try { loop {
        ...
        printf("Running CookiecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CookiecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CookiecoinMiner, pwallet));
}
